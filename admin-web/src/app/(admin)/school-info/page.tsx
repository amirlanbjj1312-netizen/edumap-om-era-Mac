...
const LABELS: Record<string, { en: string; kk: string }> = {
  ...
  'Добавить блок поступления': { en: 'Add admission block', kk: 'Қабылдау блогын қосу' },
  'Удалить блок': { en: 'Remove block', kk: 'Блокты жою' },
  'Блок поступления': { en: 'Admission block', kk: 'Қабылдау блогы' },
  'Блоки поступления': { en: 'Admission blocks', kk: 'Қабылдау блоктары' },
  ...
  'Как подать': { en: 'How to apply', kk: 'Қалай өтініш беру керек' },
  'Кого принимаете': { en: 'Who you accept', kk: 'Кімді қабылдайсыз' },
  'Что нужно родителю': { en: 'What parents need', kk: 'Ата-анаға не керек' },
  'Какие документы нужны': { en: 'Required documents', kk: 'Қандай құжаттар керек' },
...
> = {
  ...
  health_passport: { ru: 'Медицинская карта ребенка (форма № 026/у-3)', en: 'Child medical record (form No. 026/u-3)', kk: 'Баланың медициналық картасы (№ 026/у-3 нысаны)' },
  form_063: { ru: 'Карта профилактических прививок (форма № 063/у)', en: 'Immunization record (form No. 063/u)', kk: 'Профилактикалық егулер картасы (№ 063/у нысаны)' },
...
export default function SchoolInfoPage() {
  ...
  const stateAdmissionChannels = useMemo(
    () => normalizeListValue(getDeep(profile, 'education.admission_details.application_channel', [])),
    [profile]
  );
  const setAdmissionRules = (nextRules: any[]) => {
    updateField('education.admission_rules', nextRules);
  };
  const updateAdmissionRule = (index: number, patch: Record<string, any>) => {
    const current = admissionRules[index];
    if (!current) return;
    const nextRules = admissionRules.map((rule, ruleIndex) =>
      ruleIndex === index ? { ...rule, ...patch } : rule
    );
    setAdmissionRules(nextRules);
  };
  const updateAdmissionRuleLocalizedField = (
    ...
    const current = admissionRules[index];
    if (!current) return;
    updateAdmissionRule(index, {
      [field]: {
        ...(current[field] || { ru: '', en: '', kk: '' }),
        [contentLocale]: value,
      },
    });
  };
  const updateAdmissionRuleGradeRange = (
    index: number,
    patch: Partial<{ from_grade: string; to_grade: string }>
  ) => {
    const current = admissionRules[index];
    if (!current) return;
    updateAdmissionRule(index, {
              ...
              {useStateAdmissionFlow ? (
                <>
                  ...
                  <div className="teacher-list">
                    <div className="teacher-card">
                      ...
                      <CheckboxGroup
                        label="Канал подачи"
                        options={STATE_ADMISSION_CHANNEL_OPTIONS}
                        values={stateAdmissionChannels}
                        onChange={(next: string[]) =>
                          updateField('education.admission_details.application_channel', next)
                        }
                      />
                      <FieldRow>
                        <Input
                    ...
                    <div className="teacher-card">
                      ...
                      <FieldRow>
                        <Input
                          ...
                          type="date"
                          value={String(
                            getDeep(profile, 'education.admission_details.document_deadlines', '') || ''
                          )}
                          onChange={(value: string) =>
                      ...
                      <FieldRow>
                        <TextArea
                          ...
                          value={String(
                            getDeep(
                              profile,
                              `education.admission_details.documents_detail.${contentLocale}`,
                              ''
                            ) || ''
                          )}
                          onChange={(value: string) =>
                        ...
                        />
                        <TextArea
                          label="Комментарий для родителей"
                          rows={4}
                          value={String(
                            getDeep(
                              profile,
                              `education.admission_details.parent_comment.${contentLocale}`,
                              ''
                            ) || ''
                          )}
                          onChange={(value: string) =>
                ...
                <div className="teacher-list">
                  {admissionRules.map((rule, index) => (
                    <div key={String(rule.id || `admission-rule-${index}`)} className="teacher-card">
                      <div className="teacher-card-head">
                        <h3>{`${t('Блок поступления')} #${index + 1}: ${formatAdmissionGradeLabel(rule, contentLocale)}`}</h3>
                        <button
                          type="button"
                          className="button secondary"
                          onClick={() =>
                            setAdmissionRules(
                              admissionRules.filter((_item, ruleIndex) => ruleIndex !== index)
                            )
                          }
                        >
                          {t('Удалить блок')}
                      ...
                      <FieldRow>
                        <Input
                          ...
                          value={String(rule?.title?.[contentLocale] || '')}
                          onChange={(value: string) =>
                            updateAdmissionRuleLocalizedField(index, 'title', value)
                          }
                        />
                        <Select
                          label="С класса"
                          value={String(rule.from_grade || '')}
                          onChange={(value: string) =>
                            updateAdmissionRuleGradeRange(index, { from_grade: value })
                          }
                          options={[
                            { value: '', label: t('Не выбрано') },
                            ...SCHOOL_GRADE_OPTIONS.map((grade) => ({
                        ...
                        <Select
                          ...
                          value={String(rule.to_grade || '')}
                          onChange={(value: string) =>
                            updateAdmissionRuleGradeRange(index, { to_grade: value })
                          }
                          options={[
                            { value: '', label: t('Не выбрано') },
                            ...SCHOOL_GRADE_OPTIONS.map((grade) => ({
                      ...
                      <FieldRow>
                        <Input
                          label="Сроки подачи документов"
                          type="date"
                          value={String(rule.deadline || '')}
                          onChange={(value: string) =>
                            updateAdmissionRule(index, { deadline: value })
                          }
                        />
                      </FieldRow>
                      <CheckboxGroup
                      ...
                      <FieldRow>
                        <TextArea
                          ...
                          onChange={(value: string) =>
                            updateAdmissionRuleLocalizedField(index, 'stages', value)
                          }
                        />
                      </FieldRow>
                      <FieldRow>
                      ...
                      <FieldRow>
                        <TextArea
                          label="Комментарий для родителей"
                          rows={3}
                          value={String(rule?.comment?.[contentLocale] || '')}
                          onChange={(value: string) =>
