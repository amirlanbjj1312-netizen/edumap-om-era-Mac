...
const normalizeText = (value, locale = 'ru') => {
  if (!value) return '';
  if (typeof value === 'string') return value.trim();
  if (typeof value === 'object') {
    return String(value[locale] || value.ru || value.en || '').trim();
  }
  return String(value).trim();
};

const toChatSchoolPayload = (school, locale = 'ru') => ({
...
const buildAiRouter = (config) => {
  ...
  router.post('/school-chat', async (req, res, next) => {
    ...
    const locale = ['ru', 'en', 'kk'].includes(req.body?.locale) ? req.body.locale : 'ru';
    ...
    if (schoolIds.length) {
      try {
        ...
        const selected = Array.from(new Set(schoolIds))
          ...
          .filter(Boolean)
          .slice(0, MAX_SCHOOLS)
          .map((profile) => toChatSchoolPayload(profile, locale));
        if (!selected.length) {
