...
const calculateSchoolQualityScore = (profile) => {
  const monthlyFee = toNumberOrNull(profile?.finance?.monthly_fee) || 0;
  const photosCount = splitToList(profile?.media?.photos).length;
  const programsCount =
    splitToList(profile?.education?.programs).length +
    splitToList(profile?.education?.curricula?.national).length +
    splitToList(profile?.education?.curricula?.international).length +
    splitToList(profile?.education?.curricula?.additional).length;
  const clubsCount =
    splitToList(profile?.services?.clubs).length +
    splitToList(profile?.services?.clubs_unified).length;
  const hasPhone = Boolean(String(profile?.basic_info?.phone || '').trim());
  const hasAccreditation =
    Boolean(String(profile?.basic_info?.license_details?.number || '').trim()) ||
    hasContent(profile?.media?.accreditation) ||
    hasContent(profile?.media?.certificates);
  const rating = Math.max(0, toNumberOrNull(profile?.system?.rating) || toNumberOrNull(profile?.reviews?.average_rating) || 0);
...
const toSchoolCard = (profile, locale, t) => {
  ...
  return {
    ...
    qualityScore: calculateSchoolQualityScore(profile),
    updatedAt,
    promotion: {
      isPromotedActive,
