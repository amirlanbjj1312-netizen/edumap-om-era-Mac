...
const DetailRow = ({ label, value, rowStyle, labelStyle, valueStyle }) => {
  ...
  const normalizedLabel = normalizeDisplayValue(label, locale);
  const normalizedValue = normalizeDisplayValue(value, locale);
...
const formatSchoolPrice = (amount, paymentLabel, locale) => {
  ...
  const formattedAmount = Number.isFinite(numeric)
    ...
    : raw;
  if (!paymentLabel) return `${formattedAmount} ₸`;
  return `${formattedAmount} ₸ / ${paymentLabel}`;
};
...
const formatFeeRuleGrades = (fromGrade, toGrade, locale) => {
  ...
  if (locale === 'en') {
    return sameGrade ? `Grade ${start}` : `Grades ${start}-${end}`;
  }
  if (locale === 'kk') {
    return sameGrade ? `${start} сынып` : `${start}-${end} сынып`;
  }
...
// Overlay shown over blocked sections for guest users
const GuestGate = ({ isGuest, onUnlock, children, style }) => {
  const { t } = useLocale();
    ...
    <View style={[{ position: 'relative' }, style]}>
      ...
      <Pressable
        ...
        style={{
          ...
          borderRadius: 16, gap: 8, paddingVertical: 20,
        }}
      >
        <View style={{ width: 44, height: 44, borderRadius: 22, backgroundColor: '#EEF2FF', alignItems: 'center', justifyContent: 'center' }}>
          <LockClosedIcon size={20} color="#2563EB" />
        </View>
        <Text style={{ fontFamily: 'exoSemibold', fontSize: 14, color: '#111827', textAlign: 'center', paddingHorizontal: 24 }}>
          {t('guest.lockedContent')}
        </Text>
        <View style={{ backgroundColor: '#2563EB', paddingHorizontal: 20, paddingVertical: 8, borderRadius: 12 }}>
          <Text style={{ fontFamily: 'exoSemibold', fontSize: 13, color: '#fff' }}>{t('guest.signInBtn')}</Text>
        </View>
      </Pressable>
    </View>
  );
...
export default function SchoolDetailScreen() {
  ...
  const schoolPhone = useMemo(() => {
    const extraPhones = Array.isArray(basic_info.phones)
      ? basic_info.phones.map((item) => item?.value || item?.phone || '')
      : [];
    const candidates = [
      basic_info.phone,
    ...
    return match ? match.trim() : '';
  }, [basic_info.phone, basic_info.phones, profile?.contact_info?.phone]);
  const schoolPhoneUrl = buildPhoneUrl(schoolPhone);
  ...
  const handleConsultPress = async () => {
    if (whatsappChatUrl) {
      await openSocialLink(whatsappChatUrl);
      return;
    }
    if (schoolPhoneUrl) {
      try {
        const canOpen = await Linking.canOpenURL(schoolPhoneUrl);
        if (canOpen) {
          await Linking.openURL(schoolPhoneUrl);
          return;
        }
      } catch (_) {
      ...
      Array.from(
        new Set(
          (Array.isArray(finance?.payment_options) ? finance.payment_options : [])
            .map((item) => translateLabel(t, PAYMENT_LABEL_KEYS, item) || normalizeDisplayValue(item))
            .filter(Boolean)
        )
      ),
    [finance?.payment_options, t]
  );
  ...
  const hasFinanceDetails =
    ...
    Boolean(financeDiscounts) ||
    Boolean(financeGrants) ||
    Boolean(includedInTuition) ||
    Boolean(extraFees) ||
    Boolean(financeComment) ||
    Boolean(finance?.free_places);
  const teamRows = [
    normalizeDisplayValue(basic_info?.team?.principal, locale)
      ? {
          label: t('schoolDetail.field.principal'),
          value: basic_info.team.principal,
        }
      : null,
    normalizeDisplayValue(basic_info?.team?.deputy_principal, locale)
      ? {
          label: t('schoolDetail.field.deputyPrincipal'),
          value: basic_info.team.deputy_principal,
        }
      : null,
  ].filter(Boolean);
        ...
        <ScrollView contentContainerStyle={styles.container}>
          ...
          <View style={styles.quickStats}>
            {quickStats.map(({ key, iconName, label, value }) => {
              ...
              const isPriceLink = key === 'price' && hasFinanceDetails;
              return (
          <Pressable
            key={key}
            disabled={!isPriceLink}
            onPress={() =>
              isGuest
                ? navigation.navigate('SignUp')
                : navigation.navigate('SchoolPricing', { schoolId: currentSchoolId })
            }
            style={[
              styles.statItem,
              important && styles.statItemImportant,
              isPriceLink && styles.statItemLink,
            ]}
          >
            ...
            <Text
              style={[
                ...
                isPriceLink && styles.statValueLink,
              ]}
              numberOfLines={1}
            ...
            {isPriceLink ? <Text style={styles.statChevron}>›</Text> : null}
          </Pressable>
              );
            })}
      </View>
          ...
          <Pressable
            style={[styles.primaryButton, isGuest && { opacity: 0.7 }]}
            onPress={handleConsultPress}
          >
            {isGuest ? <LockClosedIcon size={16} color="#fff" style={{ marginRight: 6 }} /> : null}
          ...
          <View style={styles.featureRow}>
            <Pressable
              style={[styles.featureCard, isGuest && { opacity: 0.6 }]}
              onPress={() =>
                isGuest
                  ? navigation.navigate('SignUp')
                  : navigation.navigate('SchoolAdmission', { schoolId: currentSchoolId })
              }
            >
              <View style={styles.featureIconBadge}>
                {isGuest
                  ? <LockClosedIcon size={16} color="#1D4ED8" />
                  : <FontAwesome6 name="file-signature" size={16} color="#1D4ED8" iconStyle="solid" />}
              </View>
              <Text style={styles.featureTitle}>{t('schoolDetail.section.admission')}</Text>
            </Pressable>
            <Pressable
              style={[styles.featureCard, isGuest && { opacity: 0.6 }]}
              onPress={() =>
                isGuest
                  ? navigation.navigate('SignUp')
                  : navigation.navigate('SchoolPride', { schoolId: currentSchoolId })
              }
            >
              <View style={styles.featureIconBadge}>
                {isGuest
                  ? <LockClosedIcon size={16} color="#1D4ED8" />
                  : <FontAwesome6 name="trophy" size={16} color="#1D4ED8" iconStyle="solid" />}
              </View>
              <Text style={styles.featureTitle}>{t('schoolDetail.section.pride')}</Text>
            </Pressable>
          </View>
          ...
          {teamRows.length ? (
            <GuestGate isGuest={isGuest} onUnlock={() => navigation.navigate('SignUp')}>
              <ExpandableSection
                iconName="briefcase"
                title={t('schoolDetail.section.administration')}
          ...
          >
            {basic_info.phone ? (
              <View style={styles.detailRow}>
                <Text style={styles.detailLabel}>
                  {t('schoolDetail.field.phone')}
                </Text>
                <Pressable onPress={() => openExternalUrl(buildPhoneUrl(basic_info.phone))}>
                  <Text style={styles.detailValueLink}>{basic_info.phone}</Text>
                </Pressable>
              </View>
            ) : null}
            {Array.isArray(basic_info.phones)
              ? basic_info.phones
                  ...
                  .map((item, index) => (
                    ...
                    >
                      <Text style={styles.detailLabel}>
                        {normalizeDisplayValue(item?.label, locale) || t('schoolDetail.field.phone')}
                      </Text>
                      <Pressable onPress={() => openExternalUrl(buildPhoneUrl(item?.number))}>
                        <Text style={styles.detailValueLink}>{item?.number}</Text>
                      </Pressable>
                    </View>
                  ))
              : null}
            ...
            {basic_info.website ? (
              <View style={styles.detailRow}>
                ...
                <Pressable onPress={() => openSocialLink(basic_info.website)}>
                  <Text style={styles.detailValueLink}>{t('schoolDetail.value.tap')}</Text>
                </Pressable>
              </View>
            ) : null}
          </ExpandableSection>
            ...
            >
              ...
              <View style={styles.staffGrid}>
                {filteredStaffMembers.map((member, index) => {
                  const name =
                    normalizeDisplayValue(member?.full_name, locale) ||
                    getLocalizedText(member?.name, locale) ||
                    `${t('schoolDetail.staff.defaultName')} ${index + 1}`;
                  const subjects =
                    normalizeDisplayValue(member?.subjects, locale) ||
          ...
          <View style={styles.descriptionCard}>
            ...
            <View style={{ overflow: 'hidden', maxHeight: isGuest ? 58 : undefined, position: 'relative' }}>
              <Text style={[styles.descriptionText, isGuest && { opacity: 0.9 }]}>
            ...
            {isGuest ? (
              <LinearGradient
                ...
                style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 44, borderRadius: 8 }}
                pointerEvents="none"
              />
            ) : null}
            {isGuest ? (
              ...
              >
                <LockClosedIcon size={13} color="#2563EB" />
                <Text style={{ fontFamily: 'exoSemibold', fontSize: 13, color: '#2563EB' }}>
                  {t('guest.signInToRead')}
                </Text>
              </Pressable>
            ) : null}
          </View>
      ...
      {isChatFabVisible ? (
        <View style={styles.floatingChatWrap}>
          ...
          <View style={styles.floatingChatLabel}>
            <Text style={styles.floatingChatLabelText}>{t('schools.ai.chat')}</Text>
          </View>
        </View>
      ) : (
        <Pressable
        ...
        >
          ...
          <Text style={styles.floatingChatRestoreText}>{t('schools.ai.chat')}</Text>
        </Pressable>
      )}
      <Modal
                ...
                {isValidRemoteImage(teacherModal.member?.photo_url) ? (
                  ...
                  <View style={styles.teacherHeaderPhotoFallback}>
                    <Text style={styles.teacherHeaderPhotoFallbackText}>
                      {String(
                        normalizeDisplayValue(teacherModal.member?.full_name, locale) ||
                          getLocalizedText(teacherModal.member?.name, locale) ||
