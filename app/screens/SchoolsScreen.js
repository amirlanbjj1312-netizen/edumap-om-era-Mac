...
export default function SchoolsScreen() {
  ...
  const filteredSchools = useMemo(() => {
    ...
    decorated.sort((a, b) => {
      ...
      const qualityDiff =
        (Number(b.school?.qualityScore) || 0) - (Number(a.school?.qualityScore) || 0);
      if (qualityDiff !== 0) return qualityDiff;
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {isCompareMode ? t('schools.compare.cancel') : t('schools.compare.button')}
            </Text>
          </Pressable>
        </View>
        {isCompareMode ? (
          ...
          >
            <View className="flex-row items-center justify-between">
              <Text className="text-darkGrayText font-exoSemibold text-sm">
                {t('schools.compare.selected')
                  .replace('{count}', String(selectedCompareIds.length))
                  .replace('{limit}', String(compareLimit))}
              </Text>
              <Pressable
                onPress={async () => {
