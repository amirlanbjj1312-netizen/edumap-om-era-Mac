...
export default function AuthScreen({ initialMode = 'login' }) {
  ...
  const handlePrimaryAction = async () => {
    ...
    try {
      ...
      let otpPayload = {};
      try {
        otpResp = await fetch(buildApiUrl('/auth/send-code'), {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ email: emailInput }),
        });
        otpPayload = await otpResp.json().catch(() => ({}));
      } catch (_networkErr) {
        otpResp = null;
      }
      ...
      const isEmailNotConfigured =
        !otpResp ||
        (!otpResp.ok &&
          String(otpPayload?.error || '').toLowerCase().includes('not configured'));

      if (isEmailNotConfigured) {
        // Fallback: register directly via Supabase when OTP backend is unavailable
        ...
        setFormMessage(t('auth.success.registered'));
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      let data = {};
      try {
        resp = await fetch(buildApiUrl('/auth/send-code'), {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ email: emailInput }),
        });
      ...
      const isEmailNotConfigured =
        !resp ||
        (!resp.ok && String(data?.error || '').toLowerCase().includes('not configured'));
      ...
      if (isEmailNotConfigured) {
        ...
        setFormMessage(t('auth.success.resetSent'));
        setFormMessageTone('info');
        return;
      }
